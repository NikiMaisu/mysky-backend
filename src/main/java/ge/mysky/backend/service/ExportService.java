...
public class ExportService {
    ...
    private Object[] row(OrderResponse o) {
        return new Object[] {
            ...
            fmt(o.finishAt()),
            o.teamName(),
            o.materials().stream().map(m -> m.name() + " ×" + strip(m.squareMeters().toString()) + "m²").collect(Collectors.joining("; ")),
            o.materials().stream().map(m -> m.squareMeters().toString()).collect(Collectors.joining("; ")),
